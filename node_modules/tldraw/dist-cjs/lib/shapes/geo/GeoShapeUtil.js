...
class GeoShapeUtil extends import_editor.BaseBoxShapeUtil {
  ...
  getGeometry(shape) {
    ...
    const edges = lines ? lines.map((line) => new import_editor.Polyline2d({ points: line, isInternal: true })) : [];
...
function getIndicatorPath(shape, geometry, isZoomedOut) {
  ...
  switch (geo) {
    case "ellipse": {
      if (dash === "draw") {
        return (0, import_geo_shape_helpers.getEllipseDrawIndicatorPath)(id, w, h, strokeWidth);
      }
      return geometry.getSvgPathData(true);
    }
    case "heart": {
      return (0, import_geo_shape_helpers.getHeartPath)(w, h);
    }
    case "oval": {
      return geometry.getSvgPathData(true);
    }
    case "cloud": {
      return (0, import_geo_shape_helpers.getCloudPath)(w, h, id, size, shape.props.scale);
    }
    default: {
      ...
      if (dash === "draw" && !isZoomedOut) {
        const polygonPoints = (0, import_geo_shape_helpers.getRoundedPolygonPoints)(id, outline, 0, strokeWidth * 2 * scale, 1);
